<template>
    ...
    >
        ...
        >
            ...
            <span v-else>{{ localization.EMAIL_OTP_SEND_ACTION }}</span>
        </button>
    </form>
</template>
...
if (!context) {
    throw new Error('EmailForm must be used within AuthUIProvider')
}
...
const onSubmit = handleSubmit(async (values) => {
    try {
        ...
        toast({
            variant: 'success',
            message: localization.value.EMAIL_OTP_VERIFICATION_SENT
        })

        emit('set-email', values.email)
    } catch (error) {
        toast({
            variant: 'error',
