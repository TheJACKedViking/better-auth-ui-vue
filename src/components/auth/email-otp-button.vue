<template>
    <button
        ...
        data-slot="button"
    >
        <LockIcon v-if="view === 'EMAIL_OTP'" :class="classNames?.form?.icon" />
...
if (!context) {
    throw new Error('EmailOTPButton must be used within AuthUIProvider')
}

const { viewPaths, navigate, basePath } = context

const handleClick = () => {
    const searchParams = typeof window !== 'undefined' ? window.location.search : ''
    navigate(
        `${basePath}/${props.view === 'EMAIL_OTP' ? viewPaths.SIGN_IN : viewPaths.EMAIL_OTP}${searchParams}`
    )
}
</script>